...
class CLIPPlugin(nn.Module):
    def __init__(self, d_model, d_intermediate, dropout_rate = 0.0, activation = "gelu", eps = 1e-10):
        ...
        #with torch.no_grad():
        #    self.fc_out.weight.copy_(self.fc_in.weight.T)
...
class CLIPPreTrainedModel(PreTrainedModel):
    ...
    def post_init(self):
        return self.apply(self._init_weights)
...
class CLIPTextTransformer(nn.Module):
    ...
    def init_plugin(self, use_act = True, weight_dtype = torch.float32, device = "cuda"):
        activation = self.config.hidden_act if use_act else "linear"
        self.plugin_model = CLIPPlugin(
        ...
        self.plugin_state = True
        self.plugin_model.to(weight_dtype)
        self.plugin_model.to(device)
    ...
    def forward(
        ...
        if self.plugin_model is not None and self.plugin_state:
            last_hidden_state = self.plugin_model(last_hidden_state)
...
class CLIPTextModel(CLIPPreTrainedModel):
    ...
    def init_plugin(self, **kwargs):
        self.text_model.init_plugin(**kwargs)

    def disable_plugin(self):
        self.text_model.set_plugin_state(False)

    def enable_plugin(self):
        self.text_model.set_plugin_state(True)

    @property
    def plugin_model(self):
        return self.text_model.plugin_model
